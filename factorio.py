...
class Item(object):
    ...
    def __get_raw_materials(self, level, raw_materials_list, all_materials_dict, time):
        ...
        for mat in self.materials:
            ...
            elif type(mat) in [float, int]:
                time += mat
            else:
                all_materials_dict[level].append(str(mat))
                raw_materials_list, all_materials_dict, time = mat.__get_raw_materials(level, raw_materials_list,
                                                                                       all_materials_dict, time)
...
class CopperPlate(Item):
    def __init__(self):
        super().__init__()
        self.materials = [CopperOre(), Time(3.2)]


class IronPlate(Item):
    def __init__(self):
        super().__init__()
        self.materials = [IronOre(), Time(3.2)]


class IronGearWheel(Item):
    def __init__(self):
        super().__init__()
        self.materials = [IronPlate(), IronPlate(), Time(0.5)]
...
if __name__ == '__main__':
    ...
    total_time = science.get_raw_materials()[2]
