...
class Item(object):
    ...
    def get_raw_materials(self):
        return self.__get_raw_materials(-1, [], [], 0)

    def __get_raw_materials(self, level, raw_materials_list, all_materials_list, time):
        level += 1
        for mat in self.materials:
            if type(mat) == str:
                ...
                all_materials_list.append('|--' * level + mat.__class__.__name__)
                mat.__get_raw_materials(level, raw_materials_list, all_materials_list, time)
        return raw_materials_list, all_materials_list, time
...
class IronGearWheel(Item):
    def __init__(self):
        super().__init__()
        self.materials = [IronPlate(), IronPlate()]

class AutomationSciencePack(Item):
    def __init__(self):
        super().__init__()
        self.materials = [IronGearWheel(), CopperPlate(), Time(5)]
